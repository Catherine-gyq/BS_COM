...
public interface AccountMapper {
    ...
    // 搜索是否存在该用户
    @Select("select * from Account where usr_name=#{no} and identity= #{ident}")
    Map<String, Object> findAccount(@Param("no") String no, @Param("ident") String ident);


    // 搜索用户密码是否匹配
    @Select("select * from Account where usr_name=#{no} and password=#{pwd} and identity= #{ident}")
    Map<String, Object> findAccountPass(@Param("no") String no, @Param("pwd") String pwd, @Param("ident") String ident);

    // 重置密码
    @Update("update Account set password=#{pwd} where usr_name=#{name} and identity=#{ident}")
    Integer resetPassword(@Param("name") String name, @Param("pwd") String pwd, @Param("ident") String ident);

//    获取管理员的所有信息
    @Select("select * from Account join Admin A on Account.usr_name = A.admin_tele where usr_name=#{username} and identity= #{identity}")
    Map<String, Object> getAdminInformation(@Param("username") String username, @Param("identity") String identity);

//    获取用户的所有信息
    @Select("select * from Account join Resident R on Account.usr_name = R.tele where usr_name=#{username} and identity= #{identity}")
    Map<String, Object> getResidentInformation(@Param("username") String username, @Param("identity") String identity);

    // 新增账号
    @Insert("insert into Account(usr_name, password, identity) VALUES (#{tele}, #{pwd}, #{type})")
